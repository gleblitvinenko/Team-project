...
# ToDo: make bot parse user phone number and save it to db, if hidden - ask user to share contacts
...
async def start(message: types.Message):
    ...
    if not phone_number:
        await message.answer(
            "Please share your phone number with me, so I can contact you if needed.",
...
# @dp.message_handler(
#     lambda message: user_data.get(message.from_user.id, {}).get("state") == "waiting_first_name")
...
#
# @dp.message_handler(
#     lambda message: user_data.get(message.from_user.id, {}).get("state") == "waiting_last_name")
...
#
# @dp.message_handler(
#     lambda message: user_data.get(message.from_user.id, {}).get("state") == "waiting_phone_number")
